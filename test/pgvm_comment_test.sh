...
pgvm install 9
#status=0
#match=/^configuring PostgreSQL Version: 9.3.4 ... done.$/
#match=/^compiling ... done.$/
#match=/^installing ... done.$/

CONFIG_OPTS="--enable-cassert" pgvm install 8.4.21
#status=0
#match=/^configuring PostgreSQL Version: 8.4.21 ... done.$/

${pgvm_environments}/8.4.21/bin/pg_config
#status=0
...
## testing version 9.3.4

pgvm use 9.3.4
#status=0
#match=/^switched to 9.3.4$/
...
pgvm cluster list
#status=1
#match=/^there is no clusters in current environment \(9.3.4\)$/

pg_config --configure --version
#status=0
#match=/--prefix=.+\/environments/9.3.4/
#match=/PostgreSQL 9.3.4/

psql --version
#status=0
#match=/^psql \(PostgreSQL\) 9.3.4$/


## testing version 8.4.21

pgvm use 8.4.21
#status=0
#match=/^switched to 8.4.21$/
...
pgvm cluster list
#status=1
#match=/^there is no clusters in current environment \(8.4.21\)$/

pg_config --configure --version
#status=0
#match=/--prefix=.+\/environments/8.4.21/
#match=/--enable-cassert/
#match=/PostgreSQL 8.4.21/

psql --version
#status=0
#match=/^psql \(PostgreSQL\) 8.4.21$/


## testing clusters in 8.4.21

pgvm cluster create test
#status=0
#match=/^initializing cluster in '.+\/clusters\/8.4.21\/test'... ok!$/

pgvm cluster start test
#status=0
#match=/^starting cluster test@8.4.21$/

pgvm cluster list
#status=0
#match=/^cluster in current environment \(8.4.21\):$/
#match=/^    test  is online  at port 5433$/

pgvm cluster create latin1_cluster --encoding=latin1 --locale=en_US.iso8859-1
#status=0
#match=/^initializing cluster in '.+\/clusters\/8.4.21\/latin1_cluster'... ok!$/

## testing clusters in 9.3.4
...
#status=0
#match=/^initializing cluster in '.+\/clusters\/9.3.4\/my_cluster'... ok!$/

pgvm cluster start my_cluster
#status=0
#match=/^starting cluster my_cluster@9.3.4$/

pgvm cluster list
#status=0
#match=/^cluster in current environment \(9.3.4\):$/
#match=/^    my_cluster  is online  at port 5435$/

pgvm cluster create my_another_cluster
#status=0
#match=/^initializing cluster in '.+\/clusters\/9.3.4\/my_another_cluster'... ok!$/

pgvm cluster start my_another_cluster
#status=0
#match=/^starting cluster my_another_cluster@9.3.4$/
...
#status=0
#match=/stopping cluster test@8.4.21/

pgvm use 9.3.4
pgvm cluster stop my_cluster
#status=0
#match=/stopping cluster my_cluster@9.3.4/

pgvm cluster stop my_another_cluster
#status=0
#match=/stopping cluster my_another_cluster@9.3.4/
...
#status=0
#match=/stopping cluster to_be_removed@9.3.4/
#match=/removing 'to_be_removed' directory '.+\/clusters\/9.3.4\/to_be_removed' ... ok/
...
echo "select version()" | pgvm console my_cluster@9.3.4
#status=1
#match=/the cluster 'my_cluster' seems to be offline/

echo "select version()" | pgvm console test@8.4.21
#status=1
